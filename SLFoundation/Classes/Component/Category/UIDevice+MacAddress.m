...
+ (instancetype)sharedManager {
    static UIDevice *_manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _manager = [[UIDevice alloc] init];
    });
    return _manager;
}

-(BOOL)getDeviceIsSimulator {
    if ([[self getDeviceName] isEqualToString:@"i386Simulator"]||[[self getDeviceName] isEqualToString:@"x86_64Simulator"]) {
        return YES;
    }
    return NO;
}
...
- (NSString *)getCPUProcessor {
    return [[SLDevice sharedLibrery] getCPUProcessor] ? : @"unKnown";
}

- (const NSString *)getDeviceName {
    return [[SLDevice sharedLibrery] getDiviceName];
}

- (const NSString *)getInitialFirmware {
    return [[SLDevice sharedLibrery] getInitialVersion];
}

- (const NSString *)getLatestFirmware {
    return [[SLDevice sharedLibrery] getLatestVersion];
}
